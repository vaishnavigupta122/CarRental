...
const Hero = () => {
    ...
    return (
        <motion.div
            ...
            className='min-h-screen flex flex-col items-center justify-center gap-14 bg-light text-center pt-24 px-4'>

            <motion.h1
                initial={{ y: 50, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                transition={{ duration: 0.8, delay: 0.2 }}
                className='text-4xl md:text-5xl sm:text-5xl font-semibold  '>
                Luxury cars on Rent
            </motion.h1>
            < motion.form
                ...
                <div className='flex flex-col md:flex-row items-start md:items-center gap-10 min-md:ml-8 '>
                    ...
                    <div className='flex flex-col items-start gap-2'>
                        ...
                        <input onChange={e => setPickupDate(e.target.value)}
                            value={pickupDate}
                            type='date' id="pickup-date" required
                            min={new Date().toISOString().split('T')[0]} className='text-sm text-gray-500' />
                    </div>
